...
def dup_remove(doc):
    ...
    if len(del_ids) != 0:
        ...
        return False
# def zhq_para_select(sample,mode,MAX_P_LEN,topN):
#     """
#         主要目的是通过此函数将每个样例中的真正answer_span拿到而不是fake_answer，也不知道为啥百度不给真正answer的span
#         并且把每个Doc的段落合成一个，最后一个doc可能会没有answer，如果有的话就会一个answer。
#     """
#     if 'answers' in sample and len(sample['answer_docs']) > 0:
#         docs = []
#         for doc in sample['documents']:
#             my_doc = {}
#             my_doc['paragraphs'] = ''
#             my_doc['segmented_paragraphs'] = []
#             token_plus = 0
#             if(doc['is_selected'] is True):
#                 for i ,seg_para in enumerate(doc['segmented_paragraphs']):
#                     paragraph = doc['paragraphs'][i]
#                     if(i == doc['most_related_para']): #答案在这个段里
#                         for answer in sample['answers']:
#                             temp_para =re.sub(u"([^\u4e00-\u9fa5\u0030-\u0039\u0041-\u005a\u0061-\u007a])","",paragraph)
#                             answer_index = temp_para.find(re.sub(u"([^\u4e00-\u9fa5\u0030-\u0039\u0041-\u005a\u0061-\u007a])","",answer))
#                             if(answer_index != -1):
#                                 leng = 0 
#                                 cnt = 0 
#                                 is_start = True
#                                 temp_answer_index = {} #[doc['most_related_para']]
#                                 for token in doc['segmented_paragraphs'][int(doc['most_related_para'])]:
#                                     if(leng == answer_index and is_start): 
#                                         temp_answer_index['start_index'] = cnt + token_plus
#                                         answer_index += len(re.sub(u"([^\u4e00-\u9fa5\u0030-\u0039\u0041-\u005a\u0061-\u007a])","",answer))
#                                         is_start = False
#                                     if(leng == answer_index and not is_start):
#                                         temp_answer_index['end_index'] = cnt + token_plus
#                                         answer_index += len(answer)
#                                         break
#                                     if( is_start == True or re.sub(u"([^\u4e00-\u9fa5\u0030-\u0039\u0041-\u005a\u0061-\u007a])","",token) != ''):
#                                         leng += len(token)
#                                     cnt += 1
#                                 my_doc['answer_index']= temp_answer_index # 把其中一个答案的结果放进一个文档的
#                             elif ('answer_index' not in my_doc):
#                                 my_doc['answer_index']= None
#                     my_doc['paragraphs'] = my_doc['paragraphs'] + paragraph  
#                     my_doc['segmented_paragraphs'] = my_doc['segmented_paragraphs'] +seg_para
#                     token_plus += len(seg_para)
#                     """
#                         整合所有的段落，为什么放在后面：因为之后把ans_span改造时需要考虑到段落整合到一起了，
#                         start_index也要增加,答案所在的段前面加上了，所以只要这个段之前的段落
#                     """
#             docs.append(my_doc)
#         return docs
